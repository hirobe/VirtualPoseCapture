  ...
  public class HolisticTrackingSolution : ImageSourceSolution<HolisticTrackingGraph>
  {
    ...
    protected override void OnStartRun()
    {
      if (!runningMode.IsSynchronous())
      {
        ...
        graphRunner.OnFaceLandmarksOutput += OnFaceLandmarksOutputOfPoseSolution;
        graphRunner.OnLeftHandLandmarksOutput += OnLeftHandLandmarksOutputOfPoseSolution;
        graphRunner.OnRightHandLandmarksOutput += OnRightHandLandmarksOutputOfPoseSolution;
        graphRunner.OnPoseWorldLandmarksOutput += OnPoseWorldLandmarksOutputOfPoseSolution;
      }
    ...
    private void OnFaceLandmarksOutputOfPoseSolution(object stream, OutputEventArgs<NormalizedLandmarkList> eventArgs)
    {
      graphDataHolder.AcceptFace(eventArgs.value);
    }
    
    private void OnLeftHandLandmarksOutputOfPoseSolution(object stream, OutputEventArgs<NormalizedLandmarkList> eventArgs)
    {
      graphDataHolder.AcceptHand(eventArgs.value, true);
    }

    private void OnRightHandLandmarksOutputOfPoseSolution(object stream, OutputEventArgs<NormalizedLandmarkList> eventArgs)
    {
      graphDataHolder.AcceptHand(eventArgs.value, false); //Right
    }

    private void OnPoseWorldLandmarksOutputOfPoseSolution(object stream, OutputEventArgs<LandmarkList> eventArgs)
    {
      graphDataHolder.AcceptWorldPose(eventArgs.value);
    }
  }
}
