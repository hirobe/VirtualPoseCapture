...
public class SampleBonesSendBundle : MonoBehaviour
{
    ...
    public void SendVMD(List<VmdBone> vmdBones )
    {
        var boneBundle = new Bundle(Timestamp.Now);
        foreach (var vmdBone in vmdBones)
        {
            boneBundle.Add(new Message("/VMC/Ext/Vmd/Bone",
                vmdBone.Name.ToString(),
                vmdBone.Position.x, vmdBone.Position.y, vmdBone.Position.z,
                vmdBone.Rotation.x, vmdBone.Rotation.y, vmdBone.Rotation.z, vmdBone.Rotation.w));
        }
        uClient.Send(boneBundle);

    }

    public bool IsSendLeftHand = false;
    public bool IsSendRightHand = false;
    public bool IsSendBody = true;
    public bool IsSendFace = false;
    void Update()
    {
        //Only model updated
        ...
        if (Model != null && animator != null && uClient != null)
        {
            ...
            foreach (HumanBodyBones bone in Enum.GetValues(typeof(HumanBodyBones)))
            {
                ...
                bool isSend = false;
                isSend = (IsSendBody &&
                          bone < HumanBodyBones.LeftThumbProximal) || isSend;
                isSend = (IsSendLeftHand &&
                          bone == HumanBodyBones.LeftHand) || isSend;
                isSend = (IsSendLeftHand &&
                          ((int)bone >= (int)HumanBodyBones.LeftThumbProximal && (int)bone <= (int)HumanBodyBones.LeftLittleDistal)) || isSend;
                isSend = (IsSendRightHand &&
                          bone == HumanBodyBones.RightHand) || isSend;
                isSend = (IsSendRightHand &&
                          ((int)bone >= (int)HumanBodyBones.RightThumbProximal && (int)bone <= (int)HumanBodyBones.RightLittleDistal)) || isSend;
                if (!isSend) continue;
                */
